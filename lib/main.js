...
function hotkey_update() {
    ...
    const previous_hk_str = prefs.prefs.previous_page_hotkey;
    const next_hk_str = prefs.prefs.next_page_hotkey;

    previous_hk = hotkeys.Hotkey({
            ...
            for (let c = 0; c<reader_tab.length; c++){
                if (reader_tab[c].tab === tabs.activeTab){
                    if (reader_tab[c].number_location !== null){
                        const processed_url = new_url(reader_tab[c].tab.url,
